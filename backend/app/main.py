...
def build_heuristic_report(session: SessionState, transcript: str) -> dict[str, Any]:
    ...
    report = {
        ...
        "strengths": strengths[:4] or ["리허설 흐름을 안정적으로 확인했습니다."],
    ...
    report["speech_habits"] = speech_habits
    report.pop("script", None)
    report.pop("keyword_feedback", None)
    report.pop("presentation_material", None)
    report["delivery_match"] = {
        "spoken_words": len(transcript_tokens),
        "spoken_syllables": syllables,
    }
    report["summary"] = "말한 흐름과 구간별 기록을 바탕으로 속도, 침묵, 리듬, 말 습관을 함께 살펴봤습니다."
    if False:
    ...
    return polish_user_report_text(report)
